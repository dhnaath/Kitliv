...
const INDICES_DATA = {
  JII: {
    ...
    description: '30 saham syariah paling likuid dan berkapitalisasi besar di Bursa Efek Indonesia (Periode 2024).',
    companies: [
  ...
  JII70: {
    ...
    description: '70 saham syariah paling likuid di Bursa Efek Indonesia (Menampilkan konstituen utama).',
    companies: [
      { ticker: 'ACES', name: 'Aspirasi Hidup Indonesia Tbk.', sector: 'Consumer Cyclicals' },
      { ticker: 'ADRO', name: 'Adaro Energy Indonesia Tbk.', sector: 'Energy' },
      { ticker: 'AKRA', name: 'AKR Corporindo Tbk.', sector: 'Energy' },
  ...
  ISSI: {
    ...
    description: 'Seluruh saham syariah yang tercatat di Bursa Efek Indonesia (Menampilkan konstituen unggulan).',
    companies: [
      { ticker: 'GOTO', name: 'GoTo Gojek Tokopedia Tbk.', sector: 'Technology' },
      { ticker: 'AMMN', name: 'Amman Mineral Internasional Tbk.', sector: 'Basic Materials' },
...
export default function LocalShariaIndices() {
  ...
  const filteredCompanies = currentData.companies.filter(company => 
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-bold tracking-[0.2em] uppercase">Perusahaan Indeks Syariah Lokal</span>
          </div>
          <h2 className="text-3xl font-semibold text-slate-800">Daftar Saham Syariah</h2>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {(Object.keys(INDICES_DATA) as IndexKey[]).map((key) => (
          <button
            ...
            className={cn(
              "px-4 py-2 text-xs font-bold tracking-widest uppercase transition-colors border border-slate-200",
              selectedIndex === key 
      ...
      <div className="mb-6">
        <p className="text-sm font-medium text-slate-800/70 mb-4">
          {currentData.description}
        </p>
        <div className="relative max-w-md">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <Search className="h-4 w-4 text-slate-800/40" />
          </div>
          <input
            type="text"
            ...
            className="block w-full pl-10 pr-3 py-2 bg-white border border-slate-200 text-sm focus:outline-none focus:border-slate-200 focus:ring-0 placeholder:text-slate-800/40 transition-colors"
          />
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-200">
              <th className="py-3 px-4 text-[10px] font-bold tracking-[0.2em] uppercase text-slate-800/60">Kode</th>
              <th className="py-3 px-4 text-[10px] font-bold tracking-[0.2em] uppercase text-slate-800/60">Nama Perusahaan</th>
              <th className="py-3 px-4 text-[10px] font-bold tracking-[0.2em] uppercase text-slate-800/60">Sektor</th>
            </tr>
          </thead>
          <tbody>
            {filteredCompanies.length > 0 ? (
              filteredCompanies.map((company) => (
                <tr key={company.ticker} className="border-b border-slate-100 hover:bg-white/30 transition-colors">
                  <td className="py-3 px-4">
                    <span className="font-bold text-slate-800">{company.ticker}</span>
                  </td>
                  <td className="py-3 px-4">
                    <span className="text-sm font-medium text-slate-800">{company.name}</span>
                  </td>
                  <td className="py-3 px-4">
                    <span className="inline-block px-2 py-1 bg-slate-800/5 text-[10px] font-bold uppercase tracking-wider text-slate-800">
                      {company.sector}
                    </span>
                  </td>
                </tr>
              ...
              <tr>
                <td colSpan={3} className="py-8 text-center text-sm font-medium text-slate-800/50">
      ...
      <div className="mt-4 text-[10px] font-bold uppercase tracking-widest text-slate-800/40 text-right">
        {selectedIndex === 'JII' 
