...
export default function InvestmentTools() {
  ...
  const calculateROI = () => {
    ...
    const annualizedROI = ((Math.pow(finalValue / initialInvestment, 1 / (investmentDuration || 1))) - 1) * 100;
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <button
          ...
          className={cn(
            "px-4 py-2 text-xs font-bold tracking-widest uppercase transition-colors border border-slate-200 flex items-center gap-2",
            activeTab === 'compound' 
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8 border-b border-slate-200 pb-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-bold tracking-[0.2em] uppercase">Simulasi Investasi</span>
              </div>
              <h2 className="text-3xl font-semibold text-slate-800">Kalkulator Bunga Majemuk</h2>
            </div>
            <div className="text-sm font-medium opacity-60 text-right">
              Proyeksi Pertumbuhan Portofolio<br />(Compound Interest)
            </div>
          </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-white border border-slate-200/30 pl-4 pr-12 py-3 text-sm font-medium focus:outline-none focus:ring-0 focus:bg-white focus:border-slate-200 transition-colors"
                    />
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm font-bold opacity-50">Thn</span>
                  </div>
                </div>
                <div>
                  <label className="block text-[10px] font-bold tracking-[0.2em] uppercase text-slate-800 mb-3">
              ...
              <div className="relative z-10 space-y-6">
                ...
                <div className="space-y-2 text-sm opacity-80 pb-4 border-b border-white/10">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-green-400">+{formatIDR(compoundResult.interestEarned)}</span>
                  </div>
                </div>

                <div>
                  <span className="text-[10px] font-bold tracking-[0.2em] uppercase opacity-60 block mb-2">Estimasi Nilai Akhir</span>
                  ...
                  {user && compoundResult.total > 0 && (
                    <button
                      onClick={() => saveCompound(`Bunga Majemuk — ${formatIDR(compoundResult.total)}`, { principal, monthlyContribution, years, annualReturn }, { total: compoundResult.total, totalInvested: compoundResult.totalInvested, interestEarned: compoundResult.interestEarned })}
                    ...
                    >
                      {statusCompound === 'idle' && <><Save className="w-4 h-4" /> Simpan Hasil</>}
                      {statusCompound === 'saving' && 'Menyimpan...'}
                      {statusCompound === 'saved' && <><Check className="w-4 h-4" /> Tersimpan</>}
                    </button>
                  )}
                </div>
              </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8 border-b border-slate-200 pb-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-bold tracking-[0.2em] uppercase">Evaluasi Kinerja</span>
              </div>
              <h2 className="text-3xl font-semibold text-slate-800">Return on Investment (ROI)</h2>
            </div>
            <div className="text-sm font-medium opacity-60 text-right">
              Tingkat Pengembalian Modal<br />(Total & Disetahunkan)
            </div>
          </div>
            ...
            <div className="bg-slate-800 text-white p-8 flex flex-col justify-center relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-6">
                ...
                <div className="space-y-4 pb-6 border-b border-white/10">
                  <div>
                    <span className="text-[10px] font-bold tracking-[0.2em] uppercase opacity-60 block mb-1">Total Keuntungan / Kerugian</span>
                    <div className={cn("text-xl sm:text-2xl font-semibold font-bold", roiResult.profit >= 0 ? "text-green-400" : "text-red-400")}>
                      {roiResult.profit >= 0 ? "+" : ""}{formatIDR(roiResult.profit)}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <span className="text-[10px] font-bold tracking-[0.2em] uppercase opacity-60 block mb-2">Total ROI</span>
                    <div className={cn("text-3xl sm:text-4xl font-semibold mb-1", roiResult.roi >= 0 ? "text-white" : "text-red-400")}>
                      {roiResult.roi >= 0 ? "+" : ""}{roiResult.roi.toFixed(2)}%
                    </div>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold tracking-[0.2em] uppercase opacity-60 block mb-2">Annualized ROI</span>
                    <div className={cn("text-3xl sm:text-4xl font-semibold mb-1", roiResult.annualizedROI >= 0 ? "text-white" : "text-red-400")}>
                      {roiResult.annualizedROI >= 0 ? "+" : ""}{roiResult.annualizedROI.toFixed(2)}%
                    </div>
                  </div>
                </div>
                ...
                {user && (
                  <button
                    onClick={() => saveRoi(`ROI — ${roiResult.roi.toFixed(2)}%`, { initialInvestment, finalValue, investmentDuration }, { profit: roiResult.profit, roi: roiResult.roi, annualizedROI: roiResult.annualizedROI })}
                    disabled={statusRoi !== 'idle'}
                    className="mt-6 w-full flex items-center justify-center gap-2 py-3 px-4 bg-white/10 hover:bg-white/20 text-white text-[10px] font-bold tracking-[0.2em] uppercase transition-colors disabled:opacity-50 border border-white/20"
                  >
                    {statusRoi === 'idle' && <><Save className="w-4 h-4" /> Simpan Hasil</>}
                    {statusRoi === 'saving' && 'Menyimpan...'}
                    {statusRoi === 'saved' && <><Check className="w-4 h-4" /> Tersimpan</>}
                  </button>
                )}
              </div>
            </div>
