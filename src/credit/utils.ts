...
export const getColumnMinMax = <T, K extends keyof T>(data: T[], key: K): { min: number; max: number } => {
  let min = Infinity;
  let max = -Infinity;
  data.forEach((item) => {
    const val = item[key];
...
export const calculateCellColor = (value: number, min: number, max: number, inverse: boolean = false) => {
...
export const startOfDay = (d: Date): Date => isNaN(d.getTime()) ? new Date(NaN) : new Date(d.getFullYear(), d.getMonth(), d.getDate());

export const addDays = (d: Date, days: number): Date => new Date(d.getTime() + days * MS_DAY);

export const daysBetween = (a: Date, b: Date): number => Math.round((startOfDay(b).getTime() - startOfDay(a).getTime()) / MS_DAY);
...
export const calculateFullPayoff = (
  ...
  return parseAmount(currentDue) + parseAmount(installmentPrincipal) + parseAmount(revolvingUsage) + parseAmount(lateCharges);
};
