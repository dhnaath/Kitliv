  ...
  return (
    <Card className="p-8 mb-6 animate-in fade-in slide-in-from-top-4 duration-300">
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
        ...
        <button type="button" onClick={onCancel} className="text-slate-400 hover:text-slate-600 transition-colors bg-slate-50 p-2 rounded-full">
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4">Informasi Dasar</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <Label htmlFor="name">Nama Barang *</Label>
              <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Contoh: Susu UHT, Kertas A4" required />
            </div>
            <div>
              <Label htmlFor="category">Kategori *</Label>
              <Select id="category" value={category} onChange={(e) => setCategory(e.target.value as ItemCategory)}>
                {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </Select>
            </div>
            <div>
              <Label htmlFor="sku">SKU / Nomor Seri</Label>
              <Input id="sku" value={sku} onChange={(e) => setSku(e.target.value)} placeholder="Contoh: PRD-001" />
            </div>
            <div>
              <Label htmlFor="storageLocation">Lokasi Penyimpanan</Label>
              <Input id="storageLocation" value={storageLocation} onChange={(e) => setStorageLocation(e.target.value)} placeholder="Contoh: Rak Gudang B" />
            </div>
          </div>
        </div>
        ...
        <div>
          <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4">Masa Pakai & Spesifikasi</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <Input id="measurementValue" type="number" min="0" step="0.01" value={measurementValue} onChange={(e) => setMeasurementValue(e.target.value ? Number(e.target.value) : '')} placeholder="Misal: 100" />
              </div>
              <div>
                <Label htmlFor="measurementUnit">Satuan</Label>
                <Select id="measurementUnit" value={measurementUnit} onChange={(e) => setMeasurementUnit(e.target.value as MeasurementUnit)}>
        ...
        <div>
          <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4">Finansial & Pembelian</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <Input id="discount" type="number" min="0" value={discount} onChange={(e) => setDiscount(e.target.value ? Number(e.target.value) : '')} placeholder="0" />
            </div>
            <div>
              <Label htmlFor="purchaseStatus">Status Pembelian</Label>
              <Select id="purchaseStatus" value={purchaseStatus} onChange={(e) => setPurchaseStatus(e.target.value as PurchaseStatus)}>
        ...
        {/* Section 4: Vendor */}
        <div>
          <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4">Informasi Penjual</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <Label htmlFor="supplier">Nama Toko / Vendor</Label>
              <Input id="supplier" value={supplier} onChange={(e) => setSupplier(e.target.value)} placeholder="Contoh: PT. Makmur" />
            </div>
            <div>
              <Label htmlFor="supplierUrl">Link Toko / Produk</Label>
              <Input id="supplierUrl" type="url" value={supplierUrl} onChange={(e) => setSupplierUrl(e.target.value)} placeholder="https://..." />
            </div>
            <div>
              <Label htmlFor="supplierContact">No. HP / Kontak</Label>
              <Input id="supplierContact" type="tel" value={supplierContact} onChange={(e) => setSupplierContact(e.target.value)} placeholder="0812..." />
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <Input id="notes" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Contoh: Simpan di tempat sejuk" />
        </div>
