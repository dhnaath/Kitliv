    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 w-full">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-semibold text-slate-800 text-[13px] leading-tight">Drink Water</h3>
            <p className="text-[10px] text-slate-500 font-medium">8 glasses / day</p>
          </div>
        </div>
        <div className="text-right">
          <div className="text-[13px] font-bold text-slate-800">{completedCount}<span className="text-slate-400 text-[10px] font-normal"> / 28</span></div>
      ...
      <div className="grid grid-cols-7 gap-1.5 mt-2">
        ...
        {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => (
          <div key={`header-${i}`} className="text-[9px] font-bold text-slate-400 text-center mb-0.5">
            {d}
          </div>
        ))}
        ...
        {days.map((day, i) => (
          <motion.button
            ...
            className={cn(
              ...
              day.completed ? "bg-blue-500 shadow-sm" : "bg-slate-100 hover:bg-slate-200",
              day.isToday && !day.completed && "ring-2 ring-blue-500/40 ring-offset-1 bg-white border border-blue-200"
            )}
