  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl mx-auto w-full h-full flex flex-col">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Countdowns</h1>
          <p className="text-slate-500 text-sm mt-1">Track time until your important events.</p>
        </div>
        <button 
          onClick={() => setIsAddOpen(true)}
          className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-medium flex items-center justify-center hover:bg-blue-700 transition-colors shadow-md shadow-blue-600/20"
        >
      ...
      {events.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl bg-white/50 p-8">
           ...
           <h3 className="text-xl font-bold text-slate-800">No countdowns yet</h3>
           <p className="text-slate-500 mt-2 max-w-md text-center">Add an event to start counting down.</p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/50">
              <h2 className="text-xl font-bold text-slate-800">New Countdown</h2>
            </div>
            <form onSubmit={handleAdd} className="p-6 space-y-4">
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input 
                    type="time" name="time" defaultValue="00:00"
                    className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                  />
                </div>
              </div>
              <div className="pt-4 flex justify-end gap-3">
                ...
                <button 
                  type="submit"
                  className="px-5 py-2 rounded-xl font-medium text-white bg-blue-600 hover:bg-blue-700 shadow-md shadow-blue-600/20 transition-all"
                >
                  Create
                </button>
              </div>
...
function CountdownCard({ event, onRemove }: { event: CountdownEvent, onRemove: () => void }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-shadow relative group">
      <button 
        onClick={onRemove}
      ...
      <div className="mb-6">
        ...
        <p className="text-slate-500 text-sm flex items-center mt-1">
          ...
          {target.toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
        </p>
      </div>

      {isPast ? (
        <div className="bg-green-50 text-green-700 py-3 rounded-xl font-bold text-center">
          Event has occurred!
        </div>
      ) : (
...
function UnitBox({ value, label }: { value: number, label: string }) {
  return (
    <div className="bg-slate-50 border border-slate-100 rounded-xl p-2 flex flex-col items-center justify-center">
      <span className="font-bold text-slate-800 text-lg tabular-nums">{value}</span>
