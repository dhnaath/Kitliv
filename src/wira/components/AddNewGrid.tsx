import { User, CheckSquare, Banknote, Receipt, Calendar, Clock, Edit3, FileText, Package, Activity, Target, BookOpen, Bookmark, Lightbulb, Key, Dumbbell, Utensils, Briefcase, Music, CloudSun, Wallet, Book, Plane, ShoppingCart, Heart, Archive, CreditCard, GraduationCap, Layers, FileCheck, Globe, Film, Gamepad2, Podcast, Ticket, PenTool, Camera, Type, Code } from 'lucide-react';
...
const CATEGORIES = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'password', label: 'Password', icon: Key, color: 'text-slate-700' },
    ]
  },
  {
    id: 'health',
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'inventory', label: 'Inventory', icon: Archive, color: 'text-slate-500' },
    ]
  },
  {
    id: 'entertainment',
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'writing', label: 'Writing', icon: Type, color: 'text-slate-700' },
    ...
    <div className="w-full flex flex-col h-full overflow-y-auto scrollbar-hide py-4 px-0 gap-6 pb-20">
      {CATEGORIES.map((category) => (
        <div key={category.id} className="w-full">
          <h3 className="px-5 text-sm font-semibold text-slate-800 mb-4">{category.label}</h3>
          <div className="grid grid-cols-4 gap-y-6 gap-x-4 px-4 w-full place-items-center">
            {category.items.map((action) => (
              ...
              >
                <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-white shadow-sm flex items-center justify-center border border-slate-200 group-active:scale-95 transition-transform">
                  <action.icon className={cn("w-6 h-6 sm:w-7 sm:h-7 stroke-[1.5]", action.color)} />
                </div>
                <span className="text-[11px] sm:text-[13px] font-medium text-slate-700 text-center leading-tight">
                  {action.label}
                </span>
              </button>
            ))}
