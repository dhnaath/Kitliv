import { Users, CheckSquare, TrendingUp, Receipt, FileEdit, FolderOpen, Package, CalendarDays } from 'lucide-react';
    ...
    <div className="grid grid-cols-2 gap-4 pb-8">
      {TOOLS.map((tool) => (
        ...
        >
          ...
          <div className="relative z-10 w-12 h-12 bg-white/60 backdrop-blur-sm rounded-2xl flex items-center justify-center shadow-sm">
             <tool.icon className={cn("w-6 h-6", tool.iconColor)} />
          </div>
          
          <span className="relative z-10 text-lg font-bold text-slate-800 tracking-tight mt-auto">
            {tool.label}
          </span>
        </button>
      ))}
