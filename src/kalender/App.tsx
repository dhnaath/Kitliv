...
function MechanicalCalendar() {
  ...
  const [photo, setPhoto] = useState("https://images.unsplash.com/photo-1559839734-2b71ea197ec2?auto=format&fit=crop&q=80&w=800");
      ...
      <div className="scale-[0.45] sm:scale-[0.6] md:scale-[0.75] lg:scale-[0.9] xl:scale-100 origin-center transition-transform duration-300 relative z-10">
        ...
        >
          ...
          <div 
            ...
            style={{ 
              background: 'linear-gradient(135deg, rgba(255,255,255,0.22), rgba(255,255,255,0.05) 30%, transparent 55%)' 
            }}
          ></div>
          ...
          <div className="w-[660px] flex flex-col pt-4 relative z-10">
            ...
            <div className="relative w-full h-[80px]">
              ...
              <div className="absolute top-[42px] left-[16px] flex gap-2 font-bold text-white/90 drop-shadow-[0_1px_3px_rgba(0,0,0,0.4)] text-sm tracking-wider select-none z-0">
                {months.map((m) => (
                  <div key={m} className="w-[40px] text-center">{m}</div>
                ))}
              </div>
              ...
              >
                ...
                <div style={{ width: 56, height: 44, position: 'relative' }}>
                  ...
                  <svg width={56} height={44} className="absolute inset-0 overflow-visible pointer-events-none">
                    <defs>
                    ...
                    <rect x={8} y={8} width={40} height={28} rx={10} fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="1.5" />
                    <rect width="100%" height="100%" rx={20} fill="none" stroke="rgba(255,255,255,0.28)" strokeWidth="1.5" />
                  </svg>
                </div>
                ...
                >
                  <div className="w-2 h-2 rounded-full bg-white/70 shadow-sm"></div>
                </div>
              </motion.div>
            </div>
            ...
            <div className="relative mt-8 w-full h-[320px]">
              ...
              <div className="absolute top-[60px] left-0 grid grid-cols-[repeat(13,40px)] gap-[8px] px-[16px] font-bold text-white/90 drop-shadow-[0_1px_3px_rgba(0,0,0,0.4)] text-xl select-none z-0">
                {daysBoard.map((row, r) =>
                  row.map((day, c) => (
                    <div key={`${r}-${c}`} className="w-[40px] h-[40px] flex items-center justify-center">
                      {day}
                    </div>
                  ))
                )}
              ...
              >
                ...
                <div style={{ width: 360, height: 316, position: 'relative' }}>
                  ...
                  <svg width={360} height={316} className="absolute inset-0 overflow-visible pointer-events-none">
                    <defs>
                      <mask id="days-hole-mask">
                        <rect width="100%" height="100%" fill="white" rx={20} />
                        {holes.map((h, i) => (
                          <rect key={`mask-${i}`} x={h.x} y={h.y} width={40} height={40} rx={12} fill="black" />
                        ))}
                      </mask>
                    </defs>
                    ...
                    {["S", "M", "T", "W", "T", "F", "S"].map((day, i) => (
                      <text key={day + i} x={16 + 20 + i * 48} y={40} fill="rgba(255,255,255,0.9)" fontSize="16" fontWeight="bold" textAnchor="middle" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.4)' }}>
                        {day}
                      </text>
                    ))}
                    ...
                    {/* Inner rims */}
                    {holes.map((h, i) => (
                      <rect key={`hole-${i}`} x={h.x} y={h.y} width={40} height={40} rx={12} fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="1.5" />
                    ))}
                  </svg>
                </div>
          ...
          >
            <div className="flex-1 w-full relative group overflow-hidden rounded-[12px]" style={{ border: '1px solid rgba(255,255,255,0.3)', boxShadow: 'inset 0 4px 10px rgba(0,0,0,0.2)' }}>
              <img 
                src={photo} 
                alt="Calendar Photo" 
              ...
              <label className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center cursor-pointer backdrop-blur-md border border-white/20 rounded-[12px]">
                <div className="bg-white/20 p-4 rounded-full mb-3 shadow-lg backdrop-blur-md border border-white/30">
                  <Camera size={32} className="text-white" />
                </div>
                <span className="text-white font-bold tracking-wide drop-shadow-md">Change Photo</span>
                <input 
                  type="file" 
                  className="hidden" 
                  accept="image/*" 
...
export default function App() {
  const [activeTab, setActiveTab] = useState<"mechanical" | "tasks" | "countdown">("mechanical");

  return (
    <div className="flex flex-col h-screen w-full bg-slate-50">
      <div className="flex-none p-4 bg-white border-b border-slate-200 flex items-center justify-center gap-4 z-50 relative">
        <button 
          onClick={() => setActiveTab("mechanical")}
          className={`px-4 py-2 rounded-lg font-medium text-sm transition-colors ${activeTab === "mechanical" ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
        >
          Mechanical Calendar
        </button>
        <button 
          onClick={() => setActiveTab("tasks")}
          className={`px-4 py-2 rounded-lg font-medium text-sm transition-colors ${activeTab === "tasks" ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
        >
          Tasks Calendar
        </button>
        <button 
          onClick={() => setActiveTab("countdown")}
          className={`px-4 py-2 rounded-lg font-medium text-sm transition-colors ${activeTab === "countdown" ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
        >
          Countdown
        </button>
      </div>
      <div className="flex-1 relative overflow-hidden">
        {activeTab === "mechanical" && <MechanicalCalendar />}
        {activeTab === "tasks" && (
