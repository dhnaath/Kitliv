...
export default function App() {
  ...
  const getProgress = () => {
    const total = mode === 'work' ? workTime : mode === 'shortBreak' ? shortBreakTime : longBreakTime;
    ...
    <div className="p-6 md:p-10 max-w-2xl mx-auto w-full h-full flex flex-col items-center justify-center">
      <div className="w-full flex flex-col items-center space-y-10">
        ...
        <div className="flex bg-slate-100 p-1.5 rounded-full">
          <button
            ...
            className={cn(
              ...
              mode === 'work' ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"
            )}
          >
          ...
          <button
            ...
            className={cn(
              "px-6 py-2 rounded-full text-sm font-semibold transition-colors flex items-center gap-2",
              mode === 'shortBreak' ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"
            )}
          >
          ...
          <button
            ...
            className={cn(
              "px-6 py-2 rounded-full text-sm font-semibold transition-colors flex items-center gap-2",
              mode === 'longBreak' ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"
            )}
          >
        ...
        <div className="flex items-center gap-6">
          ...
          <button
            onClick={resetTimer}
            className="w-14 h-14 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 hover:bg-slate-200 transition-colors active:scale-95"
            title="Reset"
          >
            <RotateCcw size={24} />
          </button>
        ...
        {/* Stats */}
        <div className="flex items-center gap-2 text-sm font-medium text-slate-500 mt-10">
          <span>Cycles Completed: </span>
          <span className="text-slate-800 bg-slate-100 px-2 py-1 rounded-md">{cycles}</span>
        </div>
