...
const cardsData = [
  ...
  },
  {
    ...
    tabs: [
      ...
      { id: 'cat_tatakelola', icon: Building, color: 'text-slate-400' },
    ...
    <>
      ...
      >
        ...
        <motion.p 
           ...
           className="text-gray-400 text-sm mb-4 w-full tracking-wide"
        >
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 flex-1 min-h-0 items-center content-center ${expandedCard ? 'overflow-visible' : 'overflow-y-auto'} my-auto pr-2 pb-6 -mr-2`}>
          {cardsData.map((card, index) => {
            const isExpanded = expandedCard === card.id;
            const tCat = translations.landing.categories[card.id as keyof typeof translations.landing.categories];
            const tView = (translations as any)[card.id];
            
            return (
              <div key={card.id} className={`relative w-full h-[164px] ${isExpanded ? 'z-50' : 'z-10'}`}>
                <motion.div 
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  ...
                  className={`absolute inset-y-0 left-0 w-full my-auto bg-[#1e1e1e] rounded-2xl p-6 text-left overflow-hidden group border border-transparent hover:border-[#2a2a2a] hover:bg-[#2a2a2a] shadow-xl ${isExpanded ? 'z-50 border-[#2a2a2a] shadow-2xl overflow-y-auto max-h-[calc(100vh-80px)]' : ''} h-fit min-h-[164px]`}
                >
                  <div className="flex justify-start items-center mb-4">
                     <div className="flex items-center gap-4 text-gray-100 group-hover:text-gray-100 transition-colors">
                       ...
                       <div className="flex flex-col justify-center -mt-0.5 gap-[10pt]">
                         ...
                         <div className="text-xs text-gray-400 leading-none">{tCat.desc[lang]}</div>
                       </div>
                     </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center mt-4 pt-4 border-t border-[#2a2a2a]">
                    <AnimatePresence>
                    {!isExpanded && (
                      <motion.div initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} transition={{ duration: 0.3, ease: 'easeInOut' }} className="flex items-center gap-4 transition-colors overflow-hidden">
                    ...
                    <button type="button" onClick={(e) => toggleExpand(card.id, e)} className="ml-auto text-gray-500 hover:text-gray-100 p-1 rounded-lg hover:bg-[#2a2a2a] transition-colors">
                  ...
                  <AnimatePresence>
                     {isExpanded && (
                        ...
                        >
                           ...
                           <div className="pt-3 mt-3 grid grid-cols-2 gap-x-2 gap-y-2 text-sm text-gray-400 border-t border-[#2a2a2a]">
                               {card.tabs.map((tab, idx) => (
                                 <div key={idx} className="flex items-center justify-between group cursor-pointer hover:text-gray-100 transition-colors" onClick={(e) => { e.stopPropagation(); onNavigate(card.id, tab.id); }}>
                                     <div className={`flex items-center gap-3 ${tab.color}`}><tab.icon size={16} className="shrink-0"/> {tView.tabs[idx][lang]}</div>
        ...
        <motion.div 
          ...
          className="grid grid-cols-4 gap-2 md:gap-4 w-full pt-4 mt-auto shrink-0 z-50 bg-[#121212] relative"
        >
          <button 
             className="rounded-2xl bg-[#1e1e1e]/90 backdrop-blur border border-[#2a2a2a] flex flex-col gap-2 items-center justify-center py-2 text-gray-400 hover:text-gray-100 hover:bg-[#2a2a2a] transition-colors shadow-sm hover:shadow-md shrink-0"
             title={translations.bottomNav.more[lang]}
             onClick={onUnavailable}
          >
             <MoreHorizontal size={28} />
             <span className="text-xs font-medium">{translations.bottomNav.more[lang]}</span>
          </button>
          <button 
             className="rounded-2xl bg-[#1e1e1e]/90 backdrop-blur border border-[#2a2a2a] flex flex-col gap-2 items-center justify-center py-2 text-gray-400 hover:text-gray-100 hover:bg-[#2a2a2a] transition-colors shadow-sm hover:shadow-md shrink-0"
          ...
          >
             <Star size={28} />
             <span className="text-xs font-medium">{translations.bottomNav.favorites[lang]}</span>
          </button>
          <button 
          ...
          >
             <User size={28} />
             <span className="text-xs font-medium">{translations.bottomNav.profile[lang]}</span>
          </button>
          <button 
          ...
          >
             <Settings size={28} />
             <span className="text-xs font-medium">{translations.bottomNav.settings[lang]}</span>
          </button>
        </motion.div>
      </motion.div>
    </>
